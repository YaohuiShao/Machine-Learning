...
with tf.name_scope('conv-layer-4'):
    ...
    pool4 = tf.nn.max_pool(activ4,ksize=[1, 2, 2, 1],strides=[1, 2, 2, 1],padding='SAME', name='pool4')
                                                                           

with tf.name_scope('fully-connected-layer-1'):
    reshaped_before_out = tf.reshape(pool4,[50,2*2*64])
    fc1=fully_connected_layer(reshaped_before_out,256,256,dropout=True,keep_prob=keep_prob_fc)

with tf.name_scope('fully-connected-layer-2'):
    fc2=fully_connected_layer(fc1,256,256,dropout=True,keep_prob=keep_prob_fc)
    
with tf.name_scope('output-layer'):
    outputs = fully_connected_layer(fc2,256, train_data.num_classes,dropout=True,keep_prob=keep_prob_fc)
