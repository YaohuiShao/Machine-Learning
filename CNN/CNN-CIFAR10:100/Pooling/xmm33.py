...
with tf.name_scope('conv-layer-1'):
    ...
    pool1 = tf.nn.max_pool(activ1,ksize=[1, 3, 3, 1],strides=[1, 3, 3, 1],padding='SAME', name='pool1')
...
with tf.name_scope('conv-layer-2'):
    ...
    pool2 = tf.nn.max_pool(activ2,ksize=[1, 3, 3, 1],strides=[1, 3, 3, 1],padding='SAME', name='pool2')

with tf.name_scope('fully-connected-layer-1'):
    reshaped_before_out = tf.reshape(pool2,[50,4*4*64])
    fc1=fully_connected_layer(reshaped_before_out,1024,512,dropout=True,keep_prob=keep_prob_fc)

with tf.name_scope('fully-connected-layer-2'):
    fc2=fully_connected_layer(fc1,512,512,dropout=True,keep_prob=keep_prob_fc)
    
with tf.name_scope('output-layer'):
    outputs = fully_connected_layer(fc2,512, train_data.num_classes,dropout=True,keep_prob=keep_prob_fc)
