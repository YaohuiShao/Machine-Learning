...
with tf.name_scope('conv-layer-1'):
    ...
    pool1,row_pooling_sequence1, col_pooling_sequence1 = (activ1,pooling_ratio=
                                                                                 [1, math.sqrt(2), math.sqrt(2), 1])
...
with tf.name_scope('conv-layer-4'):
    ...
    conv4 = conv_layer(pool3,W_conv4,[1,1,1,1],64,dropout=True,keep_prob=keep_prob_conv)#16*16*64
...
with tf.name_scope('fully-connected-layer-1'):
    reshaped_before_out = tf.reshape(pool4,[50,7*7*64])
    fc1=fully_connected_layer(reshaped_before_out,3136,1568,dropout=True,keep_prob=keep_prob_fc)

with tf.name_scope('fully-connected-layer-2'):
    fc2=fully_connected_layer(fc1,1568,784,dropout=True,keep_prob=keep_prob_fc)
    
with tf.name_scope('output-layer'):
    outputs = fully_connected_layer(fc2,784, train_data.num_classes,dropout=True,keep_prob=keep_prob_fc)
