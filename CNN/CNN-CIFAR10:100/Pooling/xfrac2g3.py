...
with tf.name_scope('conv-layer-1'):
    ...
    conv1 = conv_layer(n_inputs,W_conv1,[1,1,1,1],64,dropout=False,keep_prob=keep_prob_conv)# 32*32*64
...
with tf.name_scope('fully-connected-layer-1'):
    reshaped_before_out = tf.reshape(pool2,[50,10*10*64])
    fc1=fully_connected_layer(reshaped_before_out,6400,3200,dropout=False,keep_prob=keep_prob_fc)

with tf.name_scope('fully-connected-layer-2'):
    fc2=fully_connected_layer(fc1,3200,1600,dropout=False,keep_prob=keep_prob_fc)
    
with tf.name_scope('output-layer'):
    outputs = fully_connected_layer(fc2,1600, train_data.num_classes,dropout=False,keep_prob=keep_prob_fc)
