...
with tf.name_scope('conv-layer-1'):
    ...
    pool1 = tf.nn.avg_pool(activ1,ksize=[1, 3, 3, 1],strides=[1, 2, 2, 1],padding='SAME', name='pool1')

with tf.name_scope('conv-layer-2'):
    W_conv2 = weight_variable('w_conv72',[5,5,64,64])# patch=5 in size=64, out size=64
    conv2 = conv_layer(pool1,W_conv2,[1,1,1,1],64,dropout=True,keep_prob=keep_prob_conv)#16*16*64
    batch_mean2,batch_var2 = tf.nn.moments(conv2,[0,1,2])
    norm2 = tf.nn.batch_normalization(conv2,batch_mean2,batch_var2,beta,scale,epsilon)
    activ2 = tf.nn.elu(norm2)
    pool2 = tf.nn.max_pool(activ2,ksize=[1, 3, 3, 1],strides=[1, 2, 2, 1],padding='SAME', name='pool2')

with tf.name_scope('fully-connected-layer-1'):
    reshaped_before_out = tf.reshape(pool2,[50,8*8*64])
    fc1=fully_connected_layer(reshaped_before_out,4096,2048,dropout=True,keep_prob=keep_prob_fc)

with tf.name_scope('fully-connected-layer-2'):
    fc2=fully_connected_layer(fc1,2048,1024,dropout=True,keep_prob=keep_prob_fc)
    
with tf.name_scope('output-layer'):
    outputs = fully_connected_layer(fc2,1024, train_data.num_classes,dropout=True,keep_prob=keep_prob_fc)
